...
const char *path = "../tests/test_files_phase_4/test_4_1.xml";
...
void lightInit(const char* filename)
{
    ...
    if (xmldoc.LoadFile(filename) != XML_SUCCESS) {
        cerr << "Erro ao carregar ficheiro XML\n";
        return;
    }
    ...
    XMLElement *lightdata = pworld->FirstChildElement("lights");
    if (!lightdata) return;

    XMLElement *light = lightdata->FirstChildElement("light");
    while (light) {
        const char* typeAttr = light->Attribute("type");
        if (typeAttr) {
            LightInfo info;
            info.type = typeAttr;
            light->QueryFloatAttribute("x", &info.x);
            light->QueryFloatAttribute("y", &info.y);
            light->QueryFloatAttribute("z", &info.z);
            light->QueryFloatAttribute("cutoff", &info.cutoff); 
    ...
    for (size_t i = 0; i < lights.size() && i < 8; ++i) {
        ...
        GLenum glLightId = GL_LIGHT0 + static_cast<GLenum>(i);
        glEnable(glLightId);

        GLfloat position[4];
        if (l.type == "directional") {
            position[0] = l.x; position[1] = l.y; position[2] = l.z; position[3] = 0.0f;
        } else {
            position[0] = l.x; position[1] = l.y; position[2] = l.z; position[3] = 1.0f;
        }

        glLightfv(glLightId, GL_POSITION, position);
        glLightfv(glLightId, GL_DIFFUSE, lightDiffuse);
        glLightfv(glLightId, GL_SPECULAR, lightSpecular);

        if (l.type == "spot") {
            GLfloat direction[3] = {0.0f, 0.0f, -1.0f}; 
            glLightfv(glLightId, GL_SPOT_DIRECTION, direction);
            glLightf(glLightId, GL_SPOT_CUTOFF, l.cutoff);
        }
    }
}
